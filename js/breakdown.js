...
// 检测词是否以地点/建筑/景观后缀结尾（→ 不是人名）
function hasLocationSuffix(word) {
  if (word.length > 4) return false;
  // 纯方位词：远/近/这/那/对/隔 + 处/边/面/头/方/侧/旁/端/岸/角/沿
  if (/^(?:远|近|这|那|哪|对|隔|上|下|左|右|前|后|里|外|一|两)(?:处|边|面|头|方|侧|旁|端|岸|角|沿)$/.test(word)) return true;
  // 水域/地貌后缀：边旁岸畔处坝湾潭泊渠沟滩屿洲岛坪场站港渡口
  if (/[边旁岸畔处坝湾潭泊渠沟滩屿洲岛坪场站港渡口]$/.test(word)) return true;
  // 建筑后缀：楼馆厅室园苑院府寓舍宅堂庙寺观塔碑
  if (/[楼馆厅室园苑院府寓舍宅堂庙寺观塔碑]$/.test(word)) return true;
  // 景观后缀：景野林岭谷峡坡崖壁峰顶（限3字以内）
  if (/[景野林岭谷峡坡崖壁峰顶]$/.test(word) && word.length <= 3) return true;
  // 2字江河路名：江河湖海路街道桥园场山岭谷峡坡
  if (/[江河湖海路街道桥园场山岭谷峡坡]$/.test(word) && word.length === 2) return true;
  return false;
}
...
function isChName(word) {
  ...
  // 快速排除：以地名/方位/景观后缀结尾的词 → 不是人名
  if (/[边旁岸畔处坝湾潭港渡滩屿洲岛坪场站口景野林岭谷峡坡崖壁峰顶楼馆厅室园苑院府寓堂庙寺观塔碑上下里外]$/.test(word)) return false;
  ...
  const BANNED = new Set([
    ...
    // 新增：X景 → 风景，不是人
    '江景','海景','湖景','山景','夜景','晨景','雪景','雨景','远景','近景','全景','中景',
    '街景','路景','河景','风景','美景','秋景','春景','冬景','暮景','晚景','夕景','朝景',
    '城景','市景','村景','乡景','田景','花景','树景','林景','云景','雾景','霞景','光景',
    '日景','月景','星景','天际','地平','水面','地面','对岸','彼岸','此岸','天际线',
    // 补充：常见误判词
    '坐落','远方','一侧','这头','那头','对过','斜对',
  ]);
  ...
  // 地点后缀检查
  if (hasLocationSuffix(word)) return false;
